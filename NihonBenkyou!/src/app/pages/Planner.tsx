...
export const Planner = () => {
  const {
    phase, timeLeft, totalTime, isRunning, sessionsCompleted, pomodoroSettings,
    tasks,
    startTimer, pauseTimer, resetTimer, tick, skipPhase, updateSettings,
  ...
  const handleAddTask = async () => {
    ...
    await addTask({ title: newTitle, scheduledAt: newTime, duration: newDuration });
    setNewTitle('');
    setNewDuration(15);
    ...
    <div className="max-w-2xl mx-auto w-full p-4 md:p-8 space-y-8 pb-32">
      ...
      {showSettings && (
        <div className="bg-white rounded-2xl p-6 border border-brand-200/50 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase">Work (min)</label>
              <input
                type="number"
                min={1}
                className="w-full mt-1 px-3 py-2 border rounded-xl text-sm"
                value={pomodoroSettings.workTime / 60}
                onChange={(e) => updateSettings({ workTime: Number(e.target.value) * 60 })}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => updateSettings({ breakTime: Number(e.target.value) * 60 })}
              />
            </div>
          </div>
        </div>
      ...
      <div className="bg-brand-700 rounded-3xl p-8 shadow-lg text-white flex flex-col items-center relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-brand-800">
          <div
            className="h-full bg-brand-200 transition-all duration-1000"
      ...
      {/* Schedule */}
      <div>
        <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 px-2">
          Today's Schedule
        </h2>
            ...
            {tasks.map((task) => (
              <motion.div
                ...
                className={twMerge(clsx(
                  ...
                  task.completed
                    ? 'border-brand-200/50 opacity-60 bg-brand-50/50'
              ...
              >
                ...
                <div className="flex-1 cursor-pointer" onClick={() => toggleTask(task.id)}>
                  <h3 className={twMerge(clsx('font-bold text-slate-800', task.completed && 'line-through text-slate-500'))}>
                    {task.title}
                  </h3>
            ...
            <div className="bg-white p-4 rounded-2xl border border-brand-200/50 shadow-sm space-y-3">
              ...
              <div className="flex gap-3">
                <div className="flex-1 flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wide px-1">Start Time</label>
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wide px-1">Duration (min)</label>
                  <input
                    type="number"
                    placeholder="15"
